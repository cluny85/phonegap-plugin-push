...
public class PushPlugin extends CordovaPlugin implements PushConstants {
        ...
        if (INITIALIZE.equals(action)) {
            ...
            cordova.getThreadPool().execute(new Runnable() {
                public void run() {
                    ...
                    try {
                        clearNotification(data.getJSONObject(0).getInt(ID_NOTIFICATION));
                        callbackContext.success("Clean: "+data.getJSONObject(0).getInt(ID_NOTIFICATION));
                    } catch (JSONException e) {
                        callbackContext.error(e.getMessage());
                    }
                }
    ...
    private void clearNotification(int id) {
        final NotificationManager notificationManager = (NotificationManager) cordova.getActivity().getSystemService(Context.NOTIFICATION_SERVICE);
        notificationManager.cancel(GCMIntentService.getAppName(getApplicationContext()), id);
    }
